...
class SampathPaymentService
{
    ...
    public function initiatePayment(array $paymentData): array
    {
        try {
            ...
            $callbackPath = '/api/conference/payment-callback'; // default
            
            // Check if this is a fellowship registration (by reference prefix or explicit type)
            if (isset($paymentData['event_type']) && $paymentData['event_type'] === 'fellowship') {
                $callbackPath = '/api/fellowship/payment/callback';
            } elseif (isset($paymentData['client_ref']) && str_starts_with($paymentData['client_ref'], 'FELL-')) {
                $callbackPath = '/api/fellowship/payment/callback';
            }
            ...
            Log::info('Sending payment request to bridge', [
                'endpoint' => $this->endpoint,
                'reference' => $data['reference'],
                'amount' => $data['amount']
            ]);
            ...
            Log::info('Bridge response received', [
            ...
            if ($response->successful()) {
                ...
                if (isset($result['success']) && !$result['success']) {
                    ...
                    Log::error('Payment bridge returned failure', [
                ...
                Log::error('Sampath payment HTTP error', [
                    'status' => $response->status(),
